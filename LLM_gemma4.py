...
if len(documents) == 0:
    ...
    # 💡 4비트 양자화 설정 정의 (VRAM 다이어트 핵심)
    bnb_config = BitsAndBytesConfig(
        load_in_4bit=True,                 # 4비트로 로드
        bnb_4bit_quant_type="nf4",         # 최적의 양자화 타입
        bnb_4bit_compute_dtype=torch.float16, # 연산은 float16으로 빠르게
        bnb_4bit_use_double_quant=True     # 추가 압축으로 메모리 더 아끼기
    )
